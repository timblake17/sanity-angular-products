...
const serverConnection = {
  byId(id) {
    return _observable2.default.from(_client2.default.listen('*[_id == $id]', { id: id }, { events: ['welcome', 'mutation'] })).concatMap(event => {
      return event.type === 'welcome' ? _observable2.default.from(fetchDocumentSnapshot(id)) : _observable2.default.of(event);
    });
  },

  query(query, params) {
    return _observable2.default.from(_client2.default.observable.listen(query, params || {}, { events: ['welcome', 'mutation'] })).concatMap(event => {
