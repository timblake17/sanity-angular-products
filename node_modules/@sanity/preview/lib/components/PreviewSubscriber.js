...
class PreviewSubscriber extends _react2.default.PureComponent {
  ...
  subscribe(value, type, fields) {
    ...
    const visibilityOn$ = _observable2.default.of(!document.hidden).merge(_visibilityChange2.default.map(event => !event.target.hidden));

    const inViewport$ = (0, _intersectionObservableFor2.default)(this._element).map(event => event.isIntersecting);

    this.subscription = visibilityOn$.distinctUntilChanged().switchMap(isVisible => isVisible ? inViewport$ : _observable2.default.of(false)).distinctUntilChanged().switchMap(isInViewport => {
