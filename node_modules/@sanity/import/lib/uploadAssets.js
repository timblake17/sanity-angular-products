...
let uploadAssets = (() => {
  var _ref = _asyncToGenerator(function* (assets, options) {
    ...
    const assetIds = yield pMap(assetMap.keys(), ensureAsset.bind(null, options, progress), mapOptions);

    // Loop over all documents that need asset references to be set
    const batches = yield setAssetReferences(assetMap, assetIds, options);
...
let ensureAsset = (() => {
  var _ref2 = _asyncToGenerator(function* (options, progress, assetKey, i) {
    ...
    const asset = yield client.assets.upload(type, buffer, { label, filename });
    progress();
    return asset._id;
  });
...
let getAssetIdForLabel = (() => {
  var _ref3 = _asyncToGenerator(function* (client, type, label, attemptNum = 0) {
    ...
    try {
      ...
      const assetDocId = yield client.fetch(query, { dataType, label });
      return assetDocId;
    } catch (err) {
      if (attemptNum < 3) {
  ...
  return function getAssetIdForLabel(_x7, _x8, _x9) {
